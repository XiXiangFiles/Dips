...
AppController.OBJECTS_PROPERTIES = 'OBJECTS_PROPERTIES';
...
AppController.prototype.onTab = function() {
  ...
  if (this.selectedTab == AppController.EXPORTER) {
    ...
    FactoryUtils.hide('objectsProperties');
    ...
    this.workspaceFactoryController.setBlockLibCategory(categoryXml,
        blockTypes);
  } else if (this.selectedTab ==  AppController.OBJECTS_PROPERTIES) {
    // Hide other tabs.
    FactoryUtils.hide('blockLibraryExporter');
    FactoryUtils.hide('workspaceFactoryContent');
    // Show Block Factory.
    FactoryUtils.hide('blockFactoryContent');
    FactoryUtils.show('objectsProperties');
...
AppController.prototype.assignLibraryClickHandlers = function() {
  var self = this;
  var jj = '';
  // var connectip = "192.168.50.161";
  // var url_topic = "user";

  var domain_save = 'hhttp://localhost/rewrite/dips/user1';
  ...
  document.getElementById('saveToBlockLibraryButton').addEventListener('click',
      ...
      function() {
        // for divide update and save
        var btn_content = document.getElementById('saveToBlockLibraryButton').textContent;
        var c = btn_content.substring(0, btn_content.indexOf(" "));
        // console.log(c);
        var code = document.getElementById('languagePre').textContent;
        var jp = JSON.parse(code);
        console.log(code);
        var blockType = jp.type;
        // var jsonid = blockType + makeid();
        var jsonid = blockType;
        // console.log(blockType+r);

          // var length = jp.args0.length;
          // var str = '';
          var new_json = { };
          var jsonaction = jp.args0[0].action;
          switch (jsonaction) {
            case 'create-character': {
              new_json.action = jsonaction;
              new_json.createCharacter = jp.args0[0].type;
              new_json.objectId = jsonid;
              new_json.characterId = jp.args0[0].dataTopic;
              new_json.createPosition = { };
              new_json.createPosition.x = jp.args0[0].position_x;
              new_json.createPosition.y = jp.args0[0].position_y;
              new_json.createPosition.z = jp.args0[0].position_z;
              new_json.initStatus = jp.args0[0].initStatus;
            ...
            case 'create-preset-object': {
              new_json.action = jsonaction;
              new_json.createObject = jp.args0[0].type;
              new_json.objectId = jsonid;
              new_json.initStatus = jp.args0[0].initStatus;
              new_json.createPosition = { };
              new_json.createPosition.x = jp.args0[0].position_x;
            ...
            case 'create-easycharacter': {
              new_json.action = jsonaction;
              new_json.easyCharacterName = jp.args0[0].type;
              new_json.objectId = jsonid;
              new_json.characterId = jp.args0[0].dataTopic;
              new_json.initStatus = jp.args0[0].initStatus;
              new_json.createPosition = { };
            ...
            case 'create-easyobject': {
              new_json.action = jsonaction;
              new_json.easyObjectName = jp.args0[0].type;
              new_json.objectId = jsonid;
              new_json.initStatus = jp.args0[0].initStatus;
              new_json.createPosition = { };
              new_json.createPosition.x = jp.args0[0].position_x;
            ...
            default: { // background
              new_json.action = jsonaction;
              new_json.createBackground = jp.args0[0].type;
              new_json.objectId = jsonid;
              new_json.initStatus = jp.args0[0].initStatus;
            }
            break;
          ...
          async function demo() {
          var r = {};
          r.action = "remove-scene-object";
          r.targetObject = blockType;
            ...
            request.onload = function () {
               ...
               var options = {
                 ...
                 onSuccess: function () {
                   ...
                   client.subscribe(u2d, {qos: 1});
                   var message = new Paho.MQTT.Message(splits[0]);
                   message.destinationName = d2u;
                   client.send(message);
               ...
               request.onload = function () {
                  ...
                  var options = {
                    ...
                    onSuccess: function () {
                      ...
                      client.subscribe(u2d, {qos: 1});
                      var message = new Paho.MQTT.Message(splits[1]);
                      message.destinationName = d2u;
                      client.send(message);
                      alert("Block Update!");
                    },
...
// ------------------------------------------------------------------------------
      ...
      var savefile = $("#saveToBlockLibraryButton");
      savefile.on('click', function() {
        var txt = '';
        if (confirm("Wanna save block?") == true) {
          txt = 'Ready Saved.';
          var j = JSON.parse(jj);
          var savefilename = prompt("Please enter your filename.", j.objectId);
          if (savefilename != null) {
            var blob = new Blob([jj], {type: "text/plain;charset=utf-8"});
            saveAs(blob, savefilename);
            let user_=token.split("_");
            var url = "http://192.168.50.161:8080/wise/dips/save";
            var method = "PUT";
            var async = true;
            ...
            request_mes.content = jj;
            // console.log("Fucku!!!!!\t"+jj);
            // $.ajax({
            // type: "PUT",
            // url: "http://192.168.50.161:8080/wise/dips/save",
            // data: {"fileName":savefilename,"content":jj,"token":token},
            // cache:false,
            // success: function(result){
            //   console.log("savetest\t"+result);
            //   },
            // });
            // console.log("-----------"+JSON.stringify(request_mes));
            var request = new XMLHttpRequest();
            request.onload = function () {
               var status = request.status; // HTTP response status, e.g., 200 for "200 OK"
               var data = request.responseText; // Returned data, e.g., an HTML document.
    ...
    'click',
      function() {
        ...
        if(blockType != 'block_type') {
          ...
          var new_json = { }
          new_json.action = 'remove-scene-object';
          new_json.targetObject = blockType;
