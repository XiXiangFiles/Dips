...
WorkspaceFactoryController.prototype.exportXmlFile = function(exportMode) {
  ...
  // }
    ...
    //  var block_type = xmlDoc.getElementsByTagName('block')[1].getAttributeNode('type').value;
     ...
     // var domain_save = 'http://192.168.50.161:8080/wise/dips/save/';
     let user_=token.split("_");
     var domain_save = 'http://192.168.50.161:8080/wise/dips/save';
     var actiontype = xmlDoc.getElementsByTagName('block')[0].getAttributeNode('type').value;
     switch (actiontype) {
       case 'rule':
         {
           var a = 'create-rule';
           ...
           var if_block = xmlDoc.getElementsByTagName('statement')[0].getAttributeNode('name').value; // avatar
           var if_target = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
           var do_block = xmlDoc.getElementsByTagName('statement')[1].getAttributeNode('name').value; // do
           var do_event = xmlDoc.getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
           var then_block = xmlDoc.getElementsByTagName('statement')[2].getAttributeNode('name').value; // then
           var then_effect_count = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block').length;

           dipstounity_action.action = a;
           dipstounity_action.objectId = c_id;
           dipstounity_action.event = do_event;
           dipstounity_action.TargetCharacter = if_target;
           dipstounity_action.effects = { };
           dipstounity_action.effects = new Array();

          //  console.log("then block = "+then_effect_count);
           if (then_effect_count < 5) {
             var effect_type_1 = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value; // eg, effect_shoot
             switch (effect_type_1) {
               case 'effect-create':
                 {
                   var effect_x = xmlDoc.getElementsByTagName('field')[0].firstChild.nodeValue;
                   var effect_y = xmlDoc.getElementsByTagName('field')[1].firstChild.nodeValue;
                   var effect_z = xmlDoc.getElementsByTagName('field')[2].firstChild.nodeValue;
                   var effect_object_from = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                   var effect_object_way = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
                   var effect_object_to = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value;
                   var me = effect_object_from+"."+effect_object_way;
                   dipstounity_action.effects.push({"effect-name": effect_type_1, "createObject": effect_object_to, "createAtObjectPosition": me, "createAtWorldPosition": {"x": effect_x, "y": effect_y, "z": effect_z}});

                 }
                 break;
               case 'effect-play-audio':
                {
                  var effect_audio = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                  dipstounity_action.effects.push({"effect-name": effect_type_1, "playAudio": effect_audio});
                }
                break;
               default: { // effect_shoot
                  var effect_x = xmlDoc.getElementsByTagName('field')[0].firstChild.nodeValue;
                  var effect_y = xmlDoc.getElementsByTagName('field')[1].firstChild.nodeValue;
                  var effect_z = xmlDoc.getElementsByTagName('field')[2].firstChild.nodeValue;
                  var effect_p = xmlDoc.getElementsByTagName('field')[3].firstChild.nodeValue;
                  var effect_object_from = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                  var effect_object_way = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
                  var effect_object_to = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value;
                  dipstounity_action.effects.push({"effect-name": effect_type_1, "shootObject": effect_object_to,
                                     "shootFrom": effect_object_from+"."+effect_object_way,
                                     "shootDirection": {"x": effect_x, "y": effect_y, "z": effect_z},
                                     "shootPower": effect_p});
               }
             }
           } else {
               ...
               case 'effect-create':
                 {
                   ...
                   var effect_object_way = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
                   var effect_object_to = xmlDoc.getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value;
                   dipstounity_action.effects.push({"effect-name": effect_type_1, "createObject": effect_object_to,
                                      "createAtObjectPosition": effect_object_from+"."+effect_object_way,
                                      "createAtWorldPosition": {"x": effect_x, "y": effect_y, "z": effect_z}});
                 }
                 break;
               case 'effect-play-audio':
                {
             ...
             var next_count = xmlDoc.getElementsByTagName('next').length;
            //  console.log("next_count = " + next_count);
               for (var i = 0; i < next_count; i++) {
                 var t = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getAttributeNode('type').value;
                 switch (t) {
                   case 'effect-create':
                     {
                       var effect_x = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[0].firstChild.nodeValue;
                       var effect_y = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[1].firstChild.nodeValue;
                       var effect_z = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[2].firstChild.nodeValue;
                       var effect_object_from = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                       var effect_object_way = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
                       var effect_object_to = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value;
                       dipstounity_action.effects.push({"effect-name": t, "createObject": effect_object_to,
                                          "createAtObjectPosition": effect_object_from+"."+effect_object_way,
                                          "createAtWorldPosition": {"x": effect_x, "y": effect_y, "z": effect_z}});
                   }
                    break;
                   case 'effect-play-audio':
                    {
                      var effect_audio = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      dipstounity_action.effects.push({"effect-name": t, "playAudio": effect_audio});
                    }
                    break;
                   default: { // effect_shoot
                      var effect_x = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[0].firstChild.nodeValue;
                      var effect_y = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[1].firstChild.nodeValue;
                      var effect_z = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[2].firstChild.nodeValue;
                      var effect_p = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[3].firstChild.nodeValue;
                      var effect_object_from = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      var effect_object_way = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[1].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      var effect_object_to = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[2].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      dipstounity_action.effects.push({"effect-name": t, "shootObject": effect_object_to,
                                         "shootFrom": effect_object_from+"."+effect_object_way,
                                         "shootDirection": {"x": effect_x, "y": effect_y, "z": effect_z},
                                         "shootPower": effect_p});
                   }
           ...
           cr = JSON.stringify(dipstounity_action);

           console.log("rule "+cr);
           var txt = '';
           if (confirm("Wanna save block?") == true) {
             txt = 'Ready Saved.';
             var savefilename = prompt("Please enter your filename(ex, test)", "test");
             if (savefilename != null) {
              //  var blob = new Blob([cr], {type: "text/plain;charset=utf-8"});
              //  saveAs(blob, savefilename);
                var url = domain_save;
        		    ...
        		    var request_mes = {};
        		    request_mes.token = token;
        		    request_mes.fileName = savefilename+".txt";
        		    request_mes.content = cr;
                ...
                // $.ajax({
                // type: "PUT",
                // url: "http://192.119.163.193:8080/wise/dips/save",
                // data: {"fileName":savefilename,"content":cr,"token":token},
                // cache:false,
                // success: function(result){
                //   console.log("savetest\t"+result);
                //   alert("Saved!");
                //   },
                // });
        		    request.open(method, url, async);
        		    request.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        		    request.send(JSON.stringify(request_mes));
           ...
           var url = 'http://192.168.50.161:8080/wise/dips';
           var method = "POST";
           var mes = {};
           mes.token = "user1_testToken";
           var async = true;
           ...
           request.onload = function () {
              ...
              var cloudserver_ip = resp.queue.host;
              var cloudserver_ws = resp.queue.ws;
              var u2d = resp.modulesTopic.toDips;
              var d2u = resp.modulesTopic.toUnity;
              u2d = u2d.substring(u2d.indexOf("/")+1, u2d.length);
              d2u = d2u.substring(d2u.indexOf("/")+1, d2u.length);

              var wsbroker = cloudserver_ip;
              // var wsbroker = '192.168.0.17';
              var wsport = cloudserver_ws;
              ...
              var options = {
                ...
                onSuccess: function () {
                  ...
                  client.subscribe(u2d, {qos: 1});
                  var message = new Paho.MQTT.Message(cr);
                  message.destinationName = d2u;
                  client.send(message);
                 //  saveFile();
       ...
       case 'cueQueue':
          {
            var a = 'create-cue-queue';
            var c_id = "";
            var possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
            for( var i=0; i < 7; i++ ) c_id += possible.charAt(Math.floor(Math.random() * possible.length));

            dipstounity_action.action = a;
            dipstounity_action.queueId = c_id;
            dipstounity_action.cues = { };
            dipstounity_action.cues = new Array();
            var firstcue_block = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value; // ex, cue_move_object
            var then_effect_count = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block').length;
            // console.log(then_effect_count);
            if (then_effect_count == 1 || then_effect_count == 2) {
              switch (firstcue_block) {
                case 'cue-move-object':
                  {
                    var field_x = xmlDoc.getElementsByTagName('field')[0].firstChild.nodeValue;
                    var field_y = xmlDoc.getElementsByTagName('field')[1].firstChild.nodeValue;
                    var field_z = xmlDoc.getElementsByTagName('field')[2].firstChild.nodeValue;
                    var field_t = xmlDoc.getElementsByTagName('field')[3].firstChild.nodeValue;
                    var statement_obj = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                    dipstounity_action.cues.push({"cue-name": firstcue_block, "targetObject": statement_obj,
                                       "moveVector": {"x": field_x, "y": field_y, "z": field_z},
                                       "moveTime": field_t});
                  }
                break;
                case 'cue-rotate-object':
                  {
                    ...
                    var statement_obj = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                    dipstounity_action.cues.push({"cue-name": firstcue_block, "targetObject": statement_obj,
                                       "rotateAngles": {"x": field_x, "y": field_y, "z": field_z},
                                       "rotateTime": field_t});
                  }
                break;
                case 'cue-scale-object':
                  {
                    var field_x = xmlDoc.getElementsByTagName('field')[0].firstChild.nodeValue;
                    var field_y = xmlDoc.getElementsByTagName('field')[1].firstChild.nodeValue;
                    var field_z = xmlDoc.getElementsByTagName('field')[2].firstChild.nodeValue;
                    ...
                    var statement_obj = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                    dipstounity_action.cues.push({"cue-name": firstcue_block, "targetObject": statement_obj,
                                       "scaleValues": {"x": field_x, "y": field_y, "z": field_z},
                                       "scaleTime": field_t});
                  }
                break;
                case 'cue-set-object-status':
                  {
                ...
                case 'cue-play-audio':
                  {
                    var statement_obj = xmlDoc.getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                    dipstounity_action.cues.push({"cue-name": firstcue_block, "playAudio": statement_obj});
                  }
                break;
                default: //cue-wait
                  {
                    var field = xmlDoc.getElementsByTagName('field')[0].firstChild.nodeValue;
                    dipstounity_action.cues.push({"cue-name": firstcue_block, "waitTime": field});
                  }
              }
            } else {
              // first block
              ...
              for (var i = 0; i < next_count; i++) {
                ...
                switch (t) {
                  case 'cue-move-object':
                    {
                      var field_x = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[0].firstChild.nodeValue;
                      var field_y = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[1].firstChild.nodeValue;
                      var field_z = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[2].firstChild.nodeValue;
                      var field_t = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[3].firstChild.nodeValue;
                      var statement_obj = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      dipstounity_action.cues.push({"cue-name": t, "targetObject": statement_obj,
                                         "moveVector": {"x": field_x, "y": field_y, "z": field_z},
                                         "moveTime": field_t});
                    }
                  break;
                  ...
                  case 'cue-set-object-status':
                    {
                      var field = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[0].firstChild.nodeValue;
                      var statement_obj = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      dipstounity_action.cues.push({"cue-name": t, "targetObject": statement_obj,
                                         "status": field});
                    }
                  ...
                  case 'cue-play-audio':
                    {
                      var statement_obj = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('block')[0].getElementsByTagName('statement')[0].getElementsByTagName('block')[0].getAttributeNode('type').value;
                      dipstounity_action.cues.push({"cue-name": t, "playAudio": statement_obj});
                    }
                  break;
                  default: //cue-wait
                    {
                      var field = xmlDoc.getElementsByTagName('next')[i].getElementsByTagName('field')[0].firstChild.nodeValue;
                      dipstounity_action.cues.push({"cue-name": t, "waitTime": field});
                    }
                }
              }
            ...
            console.log(cr);
            var txt = '';
            if (confirm("Wanna save cue block?") == true) {
              txt = 'Ready Saved.';
              var savefilename = prompt("Please enter your filename(ex, test)", "test");
              if (savefilename != null) {
                // var blob = new Blob([cr], {type: "text/plain;charset=utf-8"});
            ...
            async function confirm_exe() {
              txt = "Send CueQueue!";
              var exe = { };
              exe.action = "execute-queue";
              exe.targetQueue = c_id;
              var exe_json = '';
              exe_json = JSON.stringify(exe);
              console.log("1 "+exe_json);



              var r = confirm("Confirm execution?");
              if (r == true) {
                var a = '';
                if (confirm("Wanna save exe_cue block?") == true) {
                  a = 'Ready Saved.';
                  var savefilename = prompt("Please enter your filename(ex, test)", "test");
                  if (savefilename != null) {
                    // var blob = new Blob([exe_json], {type: "text/plain;charset=utf-8"});
                    // saveAs(blob, savefilename);
                    var url = domain_save;
            		    var method = "PUT";
            		    var async = true;
                ...
                console.log("Save status: " + a);
                     ...
                     var wsbroker = cloudserver_ip;
                      ...
                      var options = {
                        ...
                        onSuccess: function () {
                          console.log("Connection succeeded!");
                          client.subscribe(u2d, {qos: 1});
                          var message = new Paho.MQTT.Message(exe_json);
                          message.destinationName = d2u;
                          client.send(message);
                          alert("Block Created successfully!");
                        },
                  ...
                  txt = "You pressed Cancel(execution)!";
              }
              console.log("execution？ "+txt);
            }
            ...
            //     if (savefilename != null) {
            //       var sum = cr + "\n" + exe_json;
            //       var blob = new Blob([sum], {type: "text/plain;charset=utf-8"});
            //       saveAs(blob, savefilename);
            //       alert("Saved!");
            //     }
            //   } else {
       ...
       default:
         {
          //  var a = { };
          //  a.test = '1111';
          //  a['test-2'] = '2ssd'; // {test: "1111", test-2: "2ssd"}
...
WorkspaceFactoryController.prototype.updatePreview = function() {
  ...
  if (!this.model.options['readOnly']) {
    ...
    var tree = Blockly.Options.parseToolboxTree(
        this.generator.generateToolboxXml());
        console.log('blockly read xml to tree', tree);
