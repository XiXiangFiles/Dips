    ...
    if (!block.disabled && !block.getInheritedDisabled()) {
      ...
      switch (block.type) {
        ...
        case 'bear_BG-03':
          ...
          break;
        case 'highway-bridge-side-view-wallpaper-3':
          fields.push({
            type: block.type,
            action: 'create-background',
            initStatus: block.getFieldValue('initStatus')
        ...
        default:
            {
              var bt = block.type.split('$');
              var dynamic_type = bt[0];
              switch (dynamic_type) {
                case "ec":
                  fields.push({
                    type: bt[1],
